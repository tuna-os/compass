...
#
#   scripts/suite1/run.sh <compass> <extensions-dir> <plan.json> <report.json> [timeout]
#
...
# $XDG_DATA_HOME/compass/extensions. A private session bus with an unlocked
...
mkdir -p "$XDG_DATA_HOME/compass" "$XDG_CONFIG_HOME" "$XDG_CACHE_HOME" "$XDG_STATE_HOME" \
  "$XDG_RUNTIME_DIR" "$XDG_DATA_DIRS" "$HOME"
chmod 700 "$XDG_RUNTIME_DIR"
ln -s "$extensions" "$XDG_DATA_HOME/compass/extensions"
...
export COMPASS="$compass" PLAN="$plan" REPORT="$report" TIMEOUT="$timeout"
dbus-run-session -- sh -c '
  printf compass | gnome-keyring-daemon --unlock --components=secrets >/dev/null
  exec "$COMPASS" conformance --plan "$PLAN" --timeout "$TIMEOUT" --json > "$REPORT"
' || status=$?
