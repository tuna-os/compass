...
/// and inferring what it generated. So the shapes here are taken from the
/// figura generator (`crates/compass-figura`, ported from the C++
/// `src/lib/figura`), which is what actually emits the worker's side:
///
/// ```ts
...
mod fig_methods {
    ...
    //! it is what the committed worker bindings (`manager.ts`, generated by
    //! `crates/compass-figura` and kept current by its test) route on.
    //!
    //! WHAT IT CANNOT DO
    //!
    ...
    //! asserted separately, by reading the generated bindings.

    use super::rpc::manager;
    use std::path::{Path, PathBuf};

    const FIG: &str = "figura/manager.fig";
    const GENERATED: &str = "src/typescript/extension-manager/src/proto/manager.ts";
    ...
    fn the_wire_names_are_service_slash_method() {
        ...
        for name in [manager::LOAD, manager::UNLOAD, manager::READY] {
            assert!(
                generated.contains(&format!("case \"{name}\":")),
                "{GENERATED} does not route {name}; the wire names are no longer \
                 `<Service>/<method>` and every constant in `rpc::manager` is then wrong."
            );
        }
    }
