...
import { parseEmojiProperties } from './src/emoji-properties.ts';
...
const GLYPH_DIR = join(SCRIPTS_DIR, '..');
...
const outputs: Record<string, string> = {
	'glyph.hpp': emitHpp(provenance),
	'glyph.cpp': emitCpp(symbols, emojis, provenance),
};
for (const [rel, content] of Object.entries(outputs)) writeFileSync(join(GLYPH_DIR, rel), content);
...
console.log(`generated ${all.length} items (${emojis.length} emoji, ${symbols.length} symbols) -> glyph.cpp`);
