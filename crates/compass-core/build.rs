//! Turns the C++ glyph table into a Rust one at build time.
//!
//! `src/lib/glyph/src/glyph.cpp` is 41,879 generated lines holding every emoji
//! and curated symbol the launcher offers, produced from the Unicode Character
//! Database by `src/lib/glyph/scripts/gen.ts` and committed. Regenerating it
//! needs network access and a Node toolchain; reading it needs neither.
//!
...
fn main() {
    ...
    let table = repo.join("src/lib/glyph/src/glyph.cpp");
    let header = repo.join("src/lib/glyph/include/glyph/glyph.hpp");
    ...
    let icons = repo.join("src/server/src/services/builtin-icon/builtin-icon.cpp");
...
fn generate(source: &str, header: &str) -> String {
    ...
    format!(
        "// Generated by build.rs from src/lib/glyph/src/glyph.cpp -- do not edit.\n\
...
/// The icon names, in the order `builtin-icon.cpp` maps them.
///
/// One `{BuiltinIcon::Name, "kebab-name"}` per line; only the string matters
/// here, because the enum is a C++ detail and the name is what an extension
/// writes and what the file on disk is called.
fn generate_icons(source: &str) -> String {
    let mut names = Vec::new();
    for entry in source.split("{BuiltinIcon::").skip(1) {
        let Some(literal) = entry.split('"').nth(1) else {
            continue;
        };
    ...
    format!(
        "// Generated by build.rs from src/server/src/services/builtin-icon/builtin-icon.cpp.\n\
