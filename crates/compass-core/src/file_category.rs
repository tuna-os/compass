...
mod tests {
    ...
    const CPP: &str = "src/lib/common/include/common/file-category.hpp";
    ...
    /// The string literals of one `constexpr std::string_view NAME[] = {...}`.
    fn cpp_list(source: &str, name: &str) -> BTreeSet<String> {
        let at = source
            .find(&format!("{name}[] = {{"))
    ...
    #[test]
    fn every_list_is_the_cpps_exactly() {
        // The whole reason these can be written out here. A file whose
        // extension is in one engine's list and not the other's shows up under
        // a different filter in each, which nothing else would catch.
        let source = read_cpp();
        for (name, ours) in [
            ...
            let theirs = cpp_list(&source, name);
            assert!(
                theirs.len() >= 5,
                "parsed only {} entries from {name}; the parser has drifted",
                theirs.len()
            );
            let ours: BTreeSet<String> = ours.iter().map(|e| (*e).to_owned()).collect();
            assert_eq!(ours, theirs, "{name} disagrees with {CPP}");
        }
    }

    #[test]
    fn the_categories_are_tested_in_the_cpps_order() {
        let source = read_cpp();
        let body = source
            .split("FileCategory fileCategoryFor(")
            .nth(1)
            .expect("fileCategoryFor is still there");

        let mut seen = Vec::new();
        for line in body.lines() {
            if let Some(at) = line.find("hasExtension(ext, ")
                && let Some(name) = line[at..].split(&['(', ')', ','][..]).nth(2)
            {
                seen.push(name.trim().to_owned());
            }
        }

        let ours: Vec<String> = CATEGORY_EXTENSIONS
            .iter()
            .map(|(category, _)| format!("{category:?}_EXTENSIONS").to_uppercase())
            .collect();
        assert_eq!(seen, ours, "the order of the checks has changed");
    }
