...
pub struct InputServerFacts {
    /// `input_server.enabled` in `vicinae.json`.
    pub enabled: bool,
...
fn setcap(helper: &Path) -> String {
    format!(
        "grant it the capability the C++ helper gets at install: \
         `sudo setcap cap_dac_override+ep {}` (NixOS: the vicinae input-server module's \
         security.wrappers, then point VICINAE_INPUT_SERVER_BIN at /run/wrappers/bin/vicinae-input-server)",
        helper.display()
    )
}
...
pub fn input_server<F: FsProbe>(fs: &F, facts: &InputServerFacts) -> DoctorCheck {
    ...
    if !facts.enabled {
        return check(
            ...
            "disabled (input_server.enabled is false): snippet keywords do not expand. \
             `vicinae input-server enable` turns it on",
        );
    }
    ...
    let Some(helper) = &facts.helper else {
        return check(
            NAME,
            DoctorStatus::Warn,
            "vicinae-input-server is not installed beside the engine (or at \
             $VICINAE_INPUT_SERVER_BIN): snippet keywords will not expand",
        );
    };
