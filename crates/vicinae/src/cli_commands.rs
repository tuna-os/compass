...
/// `vicinae theme template`: every key a theme file can set, the C++'s
...
/// `vicinae server`: refuses while an engine answers, unless `replace`,
...
pub async fn server(
    ...
    if let Ok(Response::Pong { pid, .. }) = ipc::send(socket, Request::Ping).await {
        ...
        eprintln!("Killing existing vicinae server (pid {pid})...");
...
/// `vicinae cmd ls` and `vicinae cmd launch`.
///
/// # Errors
///
...
/// `vicinae script template` and `vicinae script check`.
///
/// # Errors
///
...
/// `vicinae state open`: exits 0 when the window is open, 1 when not.
///
/// # Errors
///
...
/// `vicinae logs`: the last `lines` lines of the engine's log, then, with
...
/// `vicinae theme check`: whether the theme file parses, and what was
