...
impl Updates {
    /// Compass's releases, or the feed `VICINAE_UPDATE_FEED_URL` names, and
    /// this build's version, or the one `VICINAE_UPDATE_VERSION` names.
    #[must_use]
    pub fn from_environment() -> Self {
        ...
        Self::new(
            ...
            compass_core::xdg_dirs::cache_home()
                .map(|dir| update::cache_path(&dir.join("vicinae"))),
        )
    }
