//! Running `vicinae-input-server` and talking to it.
//!
...
//! log, line by line.
//!
//! The helper is found as the C++ finds it: `$VICINAE_INPUT_SERVER_BIN`
//! first (how NixOS points at the capability-wrapped copy under
//! `/run/wrappers`), then beside the engine and in `../libexec/vicinae`.
...
pub const HELPER_PROGRAM: &str = "vicinae-input-server";

/// The variable that names the helper outright.
pub const HELPER_ENV: &str = "VICINAE_INPUT_SERVER_BIN";
...
/// Where the helper is: `$VICINAE_INPUT_SERVER_BIN`, else beside the engine
/// or in its `libexec`.
#[must_use]
    ...
    if let Some(stderr) = child.stderr.take() {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                tracing::info!(target: "vicinae::input_server::helper", "{line}");
