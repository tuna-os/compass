...
/// `vicinae ui` started by hand on a machine with no daemon should still open
...
pub fn attach(socket: &Path) -> anyhow::Result<Option<EngineLink>> {
    ...
    std::thread::Builder::new()
        .name("vicinae-window-link".to_owned())
        .spawn(move || {
...
mod tests {
    ...
    async fn a_ui_that_stops_answering_ends_the_bridge_rather_than_wedging_the_engine() {
        ...
        // hang `vicinae toggle` until someone killed the daemon.
        assert!(
            outcome.is_err(),
    ...
    async fn attaching_to_nothing_is_not_an_error() {
        // `vicinae ui` with no daemon should open an undriven launcher, not
