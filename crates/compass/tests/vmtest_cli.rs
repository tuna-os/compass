...
//! Flatpak, whose entrypoint **is** `compass`. So the arguments it passes are
//! subcommands, with no `compass` in front of them.
//!
//! A version of it wrote `compass_cli compass ping`, which runs
//! `compass compass ping`. The parser rejects that, always — so the engine
...
fn every_vm_tier_invocation_parses_as_a_real_command() {
    ...
    for (line, args) in &sites {
        ...
        // `compass` as argv[0] and the rest after it, exactly as the Flatpak
        ...
        if let Err(err) = Cli::try_parse_from(&argv) {
            panic!(
                "checks.sh:{line}: `{}` is not something this CLI accepts.\n\
                 The Flatpak entrypoint IS `compass`, so these arguments are \
                 subcommands -- writing `compass` in front of them runs \
                 `compass compass …`.\n\n{err}",
                args.join(" ")
            );
        }
...
fn the_parser_would_reject_the_mistake_this_guards_against() {
    ...
    let doubled = ["compass", "compass", "ping"];
    assert!(
        Cli::try_parse_from(doubled).is_err(),
        "the parser accepts `compass compass ping`, so the test above proves nothing"
    );

    // And the shape that must keep working, so a parser that rejected
    // everything would not look like a passing guard either.
    assert!(
        Cli::try_parse_from(["compass", "ping"]).is_ok(),
        "the parser rejects a plain `compass ping`"
    );
}
