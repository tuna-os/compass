//! `compass config`: the real binary against a throwaway `$XDG_CONFIG_HOME`.
...
fn migrate_writes_vicinae_json_and_leaves_the_cpp_file_alone() {
    let home = tempfile::tempdir().unwrap();
    let dir = home.path().join("compass");
    ...
    let written = compass_core::Config::load_from(dir.join("compass.json")).unwrap();
    ...
    assert!(
        ...
        "an existing compass.json is not replaced silently"
    );
    assert!(
    ...
    assert!(dir.join("compass.json.bak").exists());
}
...
fn theme_template_check_and_paths() {
    ...
    assert_eq!(
        ...
        Some(
            home.path()
                .join(".local/share/compass/themes")
                .to_str()
                .unwrap()
        )
    );
