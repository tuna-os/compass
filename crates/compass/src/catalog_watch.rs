...
mod tests {
    ...
    fn only_an_entry_of_an_extension_directory_or_a_manifest_matters() {
        let roots = [PathBuf::from("/data/compass/extensions")];
        let matters = |path: &str| extension_event_matters(&roots, Path::new(path));
        assert!(matters("/data/compass/extensions/clock"));
        assert!(matters("/data/compass/extensions/clock/package.json"));
        assert!(!matters("/data/compass/extensions/clock/list.js"));
        assert!(!matters("/data/compass/extensions/clock/assets/icon.png"));
