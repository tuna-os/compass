...
/// `compass/scripts` under the data home, then under each data directory.
///
/// The C++ keeps them in `vicinae/scripts`, which the startup migration moves
/// here. When it could not (both directories had a `scripts`), the old one is
/// still a directory of its own and is searched after the new one.
#[must_use]
pub fn default_directories() -> Vec<PathBuf> {
    let mut dirs: Vec<PathBuf> = Vec::new();
    if let Some(home) = compass_core::xdg_dirs::data_home() {
        dirs.push(home.join("compass/scripts"));
        let legacy = home.join("vicinae");
        if legacy.is_dir() && !legacy.is_symlink() {
            dirs.push(legacy.join("scripts"));
        }
    }
    for dir in compass_core::xdg_dirs::data_dirs() {
        let path = dir.join("compass/scripts");
