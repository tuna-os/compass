...
//! directory (`compass-notif-XXXXXX.png`) and passes that path, since the
...
mod tests {
    ...
    fn a_builtin_icon_is_drawn_into_a_tinted_square_png() {
        ...
        assert!(
            name.starts_with("compass-notif-") && name.ends_with(".png"),
            "{name}"
        );
