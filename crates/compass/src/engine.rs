//! Which engine implementation a `compass` invocation is aimed at.
//!
//! `docs/rust-engine/PLAN.md` §5 makes the two engines selectable side by side
//! for the whole migration:
//!
//! ```text
//! compass --engine=cpp     # the shipping C++ engine
//! compass --engine=rust    # this one
...
//! nobody anything.
//!
//! So: the flag parses, `COMPASS_ENGINE` overrides it, `compass doctor` reports
