...
//! `compass-file-indexer` helper, starts it, keeps it configured, asks it
...
//!   `../libexec/compass` beside the executable, which covers the same
...
pub const HELPER_PROGRAM: &str = "compass-file-indexer";
...
/// `../libexec/compass`, the installed layout the C++ candidates cover, then
/// `../lib/compass`, where Arch puts a package's internal programs.
#[must_use]
pub fn helper_candidates(exe_dir: &Path, program: &str) -> Vec<PathBuf> {
    ...
    if let Some(parent) = exe_dir.parent() {
        out.push(parent.join("libexec").join("compass").join(&name));
        out.push(parent.join("lib").join("compass").join(&name));
    }
    out
}
...
impl IndexerClient {
    ...
    fn start_process(
        ...
        let Some(program) = program else {
            tracing::warn!(
                "could not find compass-file-indexer helper binary, file search will not work"
            );
            return None;
        };
...
fn drain_stderr(stderr: Option<impl Read + Send + 'static>) {
    ...
    for line in BufReader::new(stderr).lines() {
        match line {
            Ok(line) => tracing::warn!(target: "compass-file-indexer", "{line}"),
            Err(_) => break,
        }
    }
}
...
mod tests {
    ...
    fn helper_search_covers_the_installed_layout() {
        ...
        assert_eq!(
            helper_candidates(exe_dir, "compass-file-indexer"),
            [
                PathBuf::from("/app/bin/compass-file-indexer"),
                PathBuf::from("/app/libexec/compass/compass-file-indexer"),
                PathBuf::from("/app/lib/compass/compass-file-indexer"),
            ]
        );
