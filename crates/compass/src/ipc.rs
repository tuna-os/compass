...
pub fn not_running(socket: &SocketPath, cause: &std::io::Error) -> anyhow::Error {
    ...
    anyhow::anyhow!(
        "no Compass engine is listening on {socket}\n\
         ...
         \x20 - `compass doctor` prints the socket path it resolved and everything else it \
...
fn describe(request: &Request) -> &'static str {
    match request {
        ...
        Request::ClipboardHistoryOfKind { .. } => "ClipboardHistoryOfKind",
        Request::ClipboardDetail { .. } => "ClipboardDetail",
        ...
        Request::ListShortcuts => "ListShortcuts",
        Request::SaveShortcut { .. } => "SaveShortcut",
        Request::RemoveShortcut { .. } => "RemoveShortcut",
        Request::OpenShortcut { .. } => "OpenShortcut",
        Request::ExpandShortcut { .. } => "ExpandShortcut",
        Request::ListScripts => "ListScripts",
        ...
        Request::StoreBrowse { .. } => "StoreBrowse",
        Request::StoreExtension { .. } => "StoreExtension",
        ...
        Request::ExtensionLaunchFetch { .. } => "ExtensionLaunchFetch",
        Request::ExtensionSubtitles => "ExtensionSubtitles",
        Request::ExtensionPreferences { .. } => "ExtensionPreferences",
        ...
        Request::ExpandSnippet { .. } => "ExpandSnippet",
        Request::PasteSnippet { .. } => "PasteSnippet",
...
mod tests {
    ...
    fn the_no_daemon_message_is_actionable_not_an_io_error() {
        let socket = SocketPath::exact("/run/user/1000/compass/ipc.sock");
        let err = not_running(&socket, &std::io::Error::from(ErrorKind::ConnectionRefused));
        let text = err.to_string();

        assert!(text.contains("/run/user/1000/compass/ipc.sock"));
        assert!(text.contains("start the engine"));
        assert!(text.contains("--socket"));
        assert!(text.contains("compass doctor"));
