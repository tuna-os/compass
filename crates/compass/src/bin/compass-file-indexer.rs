...
//! `compass-file-indexer`.
//!
//! Everything lives in [`compass::indexer_service`] so it can be tested; this
...
fn main() -> std::process::ExitCode {
    let Some(cache_home) = compass_core::xdg_dirs::cache_home() else {
        eprintln!("compass-file-indexer: no cache home, giving up");
        return std::process::ExitCode::FAILURE;
    };
    let Some(data_home) = compass_core::xdg_dirs::data_home() else {
        eprintln!("compass-file-indexer: no data home, giving up");
        return std::process::ExitCode::FAILURE;
    };
    ...
    if let Err(error) = prepare_file_index_database(&db_path) {
        eprintln!("compass-file-indexer: cannot prepare {db_path:?}: {error}");
        return std::process::ExitCode::FAILURE;
    }
    ...
    if SqliteWriter::open(&db_path).is_err() || SqliteReader::open(&db_path).is_err() {
        eprintln!("compass-file-indexer: cannot open {db_path:?}");
        return std::process::ExitCode::FAILURE;
    }
    ...
    let service = compass::indexer_service::IndexerService::new(indexer, pool, sink);
