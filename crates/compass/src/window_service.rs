    ...
    Some(session.toplevels.clone().ok_or_else(|| {
        ProtocolError::new(
            ...
            "window switching needs zwlr_foreign_toplevel_manager_v1 or \
             ...
             (a sandboxed client may be denied them; `compass doctor` says which)",
        )
    }))
}
        ...
        ShellError::Unavailable(availability) => ProtocolError::new(
            ...
            format!(
                "{what} needs the Compass GNOME Shell extension ({availability}). \
                 `compass doctor` shows how to install it"
            ),
        ),
        ShellError::TooOld { found, needed, .. } => ProtocolError::new(
            ...
            format!(
                "{what} needs contract v{needed} of the Compass GNOME Shell extension, and the \
                 installed one speaks v{found}. Update the extension; `compass doctor` says how"
            ),
        ),
...
mod tests {
    ...
    fn a_missing_extension_is_refused_by_name_not_as_a_crash() {
        ...
        assert!(refusal.message.contains("compass doctor"));
    }
