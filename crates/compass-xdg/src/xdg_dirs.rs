...
/// The icon directories, in precedence order: `$XDG_DATA_HOME/icons` first, then
/// each entry of `$XDG_DATA_DIRS` with `/icons` appended.
///
...
#[cfg(test)]
mod tests {
    // Tests that require env manipulation are skipped due to unsafe_code = forbid
    // They can be run manually with `cargo test -- --ignored` if unsafe code is allowed
}
