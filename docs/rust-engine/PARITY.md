...
ported and pinned, as are all five `Application` methods and all four `Command` ones: 23 of the 49
methods `figura/tsapi.fig` declares. `FileSearch`, `Clipboard`, `Application` and `Command` sit
behind traits (`FileIndexer`, `Clipboard`, `Apps`, `Commands`) that mirror the C++'s own
indirection, so the adapters are finished and tested while the backends they will call — the file
