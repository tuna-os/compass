...
| **5 — Breadth, second compositor** | parity ledger ≥ 95% green | 🔴 **37%** | `PARITY.md` holds **120 ✅, 160 ❌, 36 🟡, 6 ⏳** over 322 cells that are this port's to fill, plus 14 marked n/a. Ten builtin and service *models* landed without their views, which is why ❌ fell by 18 while ✅ rose by 2: a model without a view is 🟡 by this ledger's own rule, and the percentage barely moves. Counted by script, not by eye; the script is in the commit that last moved the figure. (Earlier revisions said 115 of 331 and, before that, 96 of 340 — the drop in ❌ is partly work and partly two rows, `permissions` and `menu-bar`, that are inside the C++ tree's macOS-only block and were never Linux work.) This remains the single largest number in the project, and it is a breadth problem rather than a hard one: most rows are individual builtins. |
