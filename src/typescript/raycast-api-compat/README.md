This package is intended to provide a Raycast compatible entrypoint to the extension API (`@vicinae/api`).
It is only meant to be bundled as part of the full javascript bundle and is not published on npm, unlike `@vicinae/api`.
...
As we continue developing the SDK, we don't want to limit ourselves to what Raycast offers so using this package as a bridge
